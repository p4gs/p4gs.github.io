...
export function localLaneBody(h: (path: string) => string, codeClass = ""): string {
  ...
  return `
    ...
    that is a permanent ceiling on evidence coverage, and the reason a well-run
    repository can still read <em>provisional</em>.</p>
    ...
    <strong>git signing key</strong> — the same <code>user.signingkey</code> /
    ...
    a namespace string and a file path, or every submission fails in a way neither side
    can see. So the agreement is written down once — in the tool's
    <code>docs/local-scan.md</code> — and mirrored here byte for byte. Both trees test
    their own copy against these values, and both pin the same digest over the block, so
    an edit on one side that is not mirrored on the other fails a test instead of
    shipping a lane that does not work.</p>
    ...
    <em>pointer</em>, and the directory reads the record, the signature and the trust
    anchor out of the public repository, so nothing a submitter types reaches the bytes
    that get verified. And <code>record-shape</code> is <code>ScanRecord</code> — the
  ...
  -s ${LOCAL_SIGNATURE_PATH} &lt; ${LOCAL_RECORD_PATH}</code></pre>
    ...
    at the scanned commit</strong> — committed content, read through the GitHub contents
    API at that ref, never anything the submitter hands us. A record whose signature
    does not verify is refused outright, exactly as a Sigstore bundle that fails
    <code>cosign verify-blob</code> is refused.</p>
    ...
    word</em> about a machine nobody can inspect, and it is the one lane whose
    local-environment verdicts count with no corroboration — the property that makes that
    acceptable is that a named human put their key behind it. CI does not need the grant:
    it has the authenticated lane, which proves strictly more. And an AI-class signer
    asserting one would contradict the invariant the signing policy itself is built on —
    an agent may draft anything and signs nothing.</p>
    ...
    <p>The same fact is why a repository anchored before this lane existed cannot submit
    yet, and why a listing in that state is told to regenerate and commit its anchor
    <em>first</em> rather than promised a one-line fix that would refuse.</p>
    ...
    verdict from <strong>every evidence source</strong> it holds — the newest
    action-lane record whose signature verified, the newest local-lane record whose
    signature verified, and the external record the directory produced itself — and
    then:</p>
    <ol>
    <li><strong>Two or more sources give different countable verdicts</strong> (pass,
    fail or gap) → the control scores <strong>gap</strong>, and carries a
    ...
    the denominator without passing. That is deliberate: erring on the side of caution is
    what removes any incentive to submit a flattering local scan, because the flattering
    answer never wins, it only ever converts a row into a gap.</p>
    ...
    top-level <code>score</code> block — a grade, a percentage, a coverage figure, computed
    on the maintainer's machine over the controls that machine had in scope. This directory
    republishes the record <strong>byte for byte</strong>, because the signature covers those
    exact bytes and rewriting them would destroy the only thing that makes the record
    evidence.</p>
    ...
    denominator, and it becomes countable the moment a CI or external record exists to
    agree or disagree with it.</p>
    ...
    the detail page, names the controls the local record actually resolved, and says so
    again when that record describes a <strong>different commit</strong> than the
    repository scan it sits beside — a local record has no expiry, and a stale one
    quietly filling holes in a much newer scan is a thing a reader has to be told
    about.</p>
